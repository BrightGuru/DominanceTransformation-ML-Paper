...
file_pheno = f"/usr/users/osatohanmwen/G2F_maize/ML_data/{trait}_ML_data.pkl"
...
params_lg = {
  'n_estimators': (3000,8000, "log-uniform"), 
  'learning_rate' : (0.005, 0.3,"log-uniform"), 

  'max_depth':(2,15,"log-uniform") , 
...
#update XGBoost parameters usingb the best estimator which was obtained by using BayesSearchCV.
...
data_pre_observed.to_csv(str(trait) +'XBM_obsv_pred.csv' + str(i) + '.csv',index=False)

data_accuracy.to_csv(str(trait) +'XBM_acc.csv' + str(i) + '.csv' ,index=False)
