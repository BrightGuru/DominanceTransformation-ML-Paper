...
file <- paste0("/usr/users/osatohanmwen/Simulation/Sim_hybrid_oligogenic/Peno_sim_hybrid.csv")
...
SNP_data <- data.frame(fread(file = "/usr/users/osatohanmwen/Simulation/Sim_hybrid_oligogenic/Geno_sim_hybrid.csv"))

SNP_data_G <- as.data.frame(SNP_data[,-1])

## Van Raden(2008) Genomic relationship matrix (additive)##
Genomic_add_matrix <- Gmatrix(SNPmatrix=as.matrix(SNP_data_G), method="VanRaden")
